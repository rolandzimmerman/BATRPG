...
/// Handles recruiting Claude: adds to party list, initializes persistent stats, and self-destructs.
...
// Check if already in party (this is a safeguard, Create event should mostly handle it for persistence)
...
if (_already_in_party_check_ue0) {
    ...
    if (variable_instance_exists(id,"dialogue_data_post_recruit")) { 
        ...
        dialogue_data = [ { name: _char_key, msg: "We're already partners!" } ]; 
    }
...
if (_can_still_recruit_locally) {
    ...
    if (variable_global_exists("party_members") && is_array(global.party_members)) {
        ...
        // Potentially play an error sound or show an error dialog
        exit; // Stop recruitment if party list is broken
    }

    // --- Initialize persistent stats for this character ---
    // (Your existing stat initialization logic - ensure scr_FetchCharacterInfo and scr_GetXPForLevel exist)
    if (variable_global_exists("party_current_stats") && ds_exists(global.party_current_stats, ds_type_map)) {
        if (!ds_map_exists(global.party_current_stats, _char_key)) { 
            ...
            if (is_struct(_base_data)) {
                ...
                var _initial_maxhp = variable_struct_get(_base_data, "hp_total") ?? 35;
                var _initial_maxmp = variable_struct_get(_base_data, "mp_total") ?? 15;
                
                var _initial_stats = {
                    hp:          _initial_maxhp, maxhp:       _initial_maxhp, 
                    mp:          _initial_maxmp, maxmp:       _initial_maxmp,
                    atk:         variable_struct_get(_base_data, "atk") ?? 8,   
                    def:         variable_struct_get(_base_data, "def") ?? 4, 
    ...
    // Change local NPC state (instance will be destroyed shortly anyway)
    can_recruit = false; 
    ...
    if (variable_instance_exists(id,"dialogue_data_recruit")) { 
        ...
        dialogue_data = [ { name: "System", msg: string(char_display_name_success) + " joined the party!" } ]; 
    }
    if(script_exists(create_dialog)) create_dialog(dialogue_data); 
    ...
    // This case (_can_still_recruit_locally is false, but not caught by _already_in_party_check_ue0)
    // implies the NPC was interacted with, recruitment was declined or some other path was taken,
    // and can_recruit was set to false for this instance *without* party addition.
    // Or, it's a state that shouldn't be reached if the instance is destroyed on recruitment.
    ...
    if (variable_instance_exists(id,"dialogue_data_post_recruit")) { 
        ...
        dialogue_data = [ { name: _char_key, msg: "Let's get going!" } ]; 
    }
    if(script_exists(create_dialog)) create_dialog(dialogue_data);
}